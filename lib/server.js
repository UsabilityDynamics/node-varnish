...
Server.prototype.toString = function() {
  ...
  opts.forEach( function( opt ) {
    ...
    if( Array.isArray( val ) ) instance.push( flag, val.join( ',' ) ); else if( typeof val == 'object' )
      Object.keys( val ).forEach( function( key ) {
        if( typeof val[key] == 'undefined' ) instance.push( flag, key ); else instance.push( flag, key + '=' + val[key] );
      } ); else if( val ) instance.push( flag, val ); else instance.push( flag );
...
Server.prototype.args = function( args ) {
  ...
  Object.keys( parsed.config ).forEach( function( arg ) {
    if( !this.config[arg] ) this.config[arg] = parsed.config[arg]; else {
      if( Array.isArray( this.config[arg] ) )
        this.config[arg] = this.config[arg].concat( parsed.config[arg] ); else if( typeof this.config[arg] == 'object' )
        util.merge( this.config[arg], parsed.config[arg] ); else this.config[arg] = parsed.config[arg];
    }
  }, this );
  return this;
};
  ...
  exec( cmd, function( err, stdout, stderr ) {
    ...
    fs.readFile( self.config['-P'], function( err, content ) {
      ...
      ps.get( content.toString(), function( err, result ) {
        ...
        try {
          self.hydrate( result ); // wtf is hydrate? - potanin@UD
        } catch( error ) {
          // debug( 'Missing Server.hydrate() method...' );
        }

        debug( 'Started Varnish [pid=%d]. Command used: [%s %s].', result.pid, result.command, result.args );

        if( !self.pid && result.pid ) {
          self.pid = parseInt( result.pid );
        }
...
function randomPidFile() {
  return [ os.tmpDir(), 'nodejs', now.getYear(), now.getMonth(), now.getDate(), '-', process.pid, '-', (Math.random() * 0x100000000 + 1).toString( 36 ), 'varnish'].join( '' );
}
